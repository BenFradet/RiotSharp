using System.Net;

namespace RiotSharpNET8
{
    /// <summary>
    /// RiotSharp exception.
    /// </summary>
    public class RiotSharpException: Exception
    {
		/// <summary>
		/// HTTP error code returned by the Riot API, causing this exception.
		/// Can be null since it might be thrown before the request is sent or if the request fails before a response is received.
		/// </summary>
		public readonly HttpStatusCode? HttpStatusCode;

        /// <summary>
        /// Initializes a new instance of the <see cref="RiotSharpException"/> class.
        /// </summary>
        /// <param name="message">The message.</param>
        /// <param name="httpStatusCode">The HTTP status code.</param>
        public RiotSharpException(string message, HttpStatusCode? httpStatusCode) : base(message)
        {
            HttpStatusCode = httpStatusCode;
        }
    }
}
