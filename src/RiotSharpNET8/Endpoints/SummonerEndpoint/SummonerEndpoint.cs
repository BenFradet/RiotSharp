...
namespace RiotSharpNET8.Endpoints.SummonerEndpoint
{
    ...
    /// </summary>
    /// <seealso cref="ISummonerEndpoint" />
    public class SummonerEndpoint : ISummonerEndpoint
    {
        ...
        public async Task<Summoner> GetSummonerBySummonerIdAsync(Region region, string summonerId)
        {
            ...
            var summoner = JsonSerializer.Deserialize<Summoner>(json); //JsonConvert.DeserializeObject<Summoner>(jsonResponse);
            if (summoner != null)
            {
        ...
        public async Task<Summoner> GetSummonerByPuuidAsync(Region region, string puuid)
        {
            ...
            var json = await _requester.CreateGetRequestAsync(string.Format(SummonerRootUrl + SummonerByPuuid, puuid), region).ConfigureAwait(false);
            var summoner = JsonSerializer.Deserialize<Summoner>(json); //JsonConvert.DeserializeObject<Summoner>(jsonResponse);
            if (summoner != null)
            {
                summoner.Region = region;
